...
const ViewStickersByGroups: React.FC<ViewStickersByGroupsProps> = ({
    checkIsStickerObtainedByUser, handleClickSticker, stickers
}) => {
            ...
            {stickersByGroups &&
                Object.keys(stickersByGroups).map((key: any) => (
                    <div key={'containerKey-' + key} style={{ display: 'grid', alignItems: 'center' }}>
                        <Typography variant='subtitle1' style={{ minWidth: 'fit-content' }}>Grupo {key}</Typography>
                        <div key={'containerCountriesGroup-' + key} style={{ display: 'grid' }}>
                            {sliceIntoChunks(stickersByGroups[key], 20).map((stickersByCountry: StickerModel[], index) => (
                                <FaderDiv key={'containerCountry-' + index} style={{ display: 'flex', overflowX: 'auto', gap: '0px 10px', padding: '5px' }}>
                                    {stickersByCountry.map((sticker: StickerModel, index: number) => (
                                        <Item
                                            key={'item-' + sticker.code}
                                            onClick={() => handleClickSticker(sticker)}
                                            style={{ cursor: "pointer", backgroundColor: checkIsStickerObtainedByUser(sticker.code) ? 'green' : '' }}
                                        >
                                            <p key={'p-' + sticker.code} style={{ margin: 5 }}>{sticker.code}</p>
                                        </Item>
                                    ))}
                                </FaderDiv>
                            ))}
